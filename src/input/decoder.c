...
static void DecoderStoreNewSeg(decoder_t *p_dec, char* p_start,char* p_end,char* p_audio_func,char* p_video_func,char* p_sync,char* p_audio_pause, char* p_video_pause)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    ...
    switch (p_dec->fmt_out.i_cat)
    {
        case VIDEO_ES:
            ...
            p_owner->b_sync = (strcmp(p_sync,SYNC_VIDEO_FLAG)==0);
            p_owner->b_paused_mask = (strcmp(p_video_pause,EMPTY_FLAG) != 0);
            p_owner->i_pausing_period = (strcmp(p_video_pause,EMPTY_FLAG) != 0)? (atoi(p_video_pause) * MICRO_SEC):-1;
            break;
        case AUDIO_ES:
        case SPU_ES:
            strcpy(p_owner->psz_rate_func,p_audio_func);
            p_owner->b_sync = (strcmp(p_sync,SYNC_AUDIO_FLAG)==0);
            p_owner->b_paused_mask = (strcmp(p_audio_pause,EMPTY_FLAG) != 0);
            p_owner->i_pausing_period = (strcmp(p_audio_pause,EMPTY_FLAG) != 0)? (atoi(p_audio_pause) * MICRO_SEC):-1;
            break;
        default:
...
static void InitCommandsArray(decoder_t *p_dec,int* commands){
    ...
    {
        case VIDEO_ES:
            commands[RATE_CMD]=INPUT_CONTROL_SET_RATE_VIDEO;
            commands[STATE_CMD]=INPUT_CONTROL_SET_STATE_VIDEO;
            commands[SYNC_CMD]=INPUT_CONTROL_SYNC_VIDEO;
            break;
        case AUDIO_ES:
        case SPU_ES:
            commands[RATE_CMD]=INPUT_CONTROL_SET_RATE_AUDIO;
            commands[STATE_CMD]=INPUT_CONTROL_SET_STATE_AUDIO;
            commands[SYNC_CMD]=INPUT_CONTROL_SYNC_AUDIO;
            break;
        default:
            msg_Err(p_dec, "unknown ES format");
            return;
...
static void DecoderApplyMask(decoder_t *p_dec,int* commands){
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    vlc_value_t val;
    ...
    if (i_last_stream >= i_seg_start && i_last_stream < i_seg_end)
    {
        if( p_owner->b_paused_mask ) //PAUSE
        {
            if( p_owner->i_pause_start == -1 || !p_owner->paused)
            {
                ...
                DecoderControlPush(p_dec,commands[RATE_CMD],&val);
            }
            else
            {
...
static void *DecoderThread(void *p_data)
{
    ...
    int commands[COMMANDS_NUM];
    InitCommandsArray(p_dec,commands);
    ...
    for (;;)
    {
        if(p_owner->p_maskfile)
        {
            vlc_fifo_Unlock(p_owner->p_fifo);
            // vlc_mutex_lock(&p_owner->lock);
            DecoderApplyMask(p_dec,commands);
            // vlc_mutex_unlock(&p_owner->lock);
            vlc_fifo_Lock(p_owner->p_fifo);
        }
