...
static mtime_t DecoderGetDisplayDate(decoder_t *p_dec, mtime_t i_ts)
{
    ...
    if (input_clock_ConvertTS(VLC_OBJECT(p_dec), p_owner->p_clock, NULL, &i_ts, NULL, INT64_MAX, 1))
    {
...
{
    assert(dec->fmt_out.audio.i_frame_length > 0 && dec->fmt_out.audio.i_bytes_per_frame > 0);
...
static int DecoderTimedWait(decoder_t *p_dec, mtime_t deadline)
{
    ...
    while (!p_owner->flushing && vlc_fifo_TimedWaitCond(p_owner->p_fifo, &p_owner->wait_timed,
                                                        deadline) == 0)
        ;
...
static void DecoderFixTs(decoder_t *p_dec, mtime_t *pi_ts0, mtime_t *pi_ts1,
                         mtime_t *pi_duration, int *pi_rate, mtime_t i_ts_bound, bool b_isVideo)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    ...
    if (*pi_ts0 > VLC_TS_INVALID)
    {
        *pi_ts0 += i_es_delay;
        ...
        if (input_clock_ConvertTS(VLC_OBJECT(p_dec), p_clock, &i_rate, pi_ts0, pi_ts1, i_ts_bound, b_isVideo))
        {
            const char *psz_name = module_get_name(p_dec->p_module, false);
            if (pi_ts1 != NULL)
                msg_Err(p_dec, "Could not convert timestamps %" PRId64 ", %" PRId64 " for %s", *pi_ts0, *pi_ts1, psz_name);
            else
    ...
    if (pi_duration)
        *pi_duration = (*pi_duration * i_rate + INPUT_RATE_DEFAULT - 1) / INPUT_RATE_DEFAULT;
...
{
    msg_Err(p_dec, "In DecoderPlaySout");
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    ...
    DecoderFixTs(p_dec, &p_sout_block->i_dts, &p_sout_block->i_pts,
                 &p_sout_block->i_length, NULL, INT64_MAX, 1);
                            ...
                            unsigned *restrict pi_lost_sum)
{
    ...
    DecoderFixTs(p_dec, &p_picture->date, NULL, NULL,
                 &i_rate, DECODER_BOGUS_VIDEO_DELAY, 1);
                            ...
                            unsigned *restrict pi_lost_sum)
{
    ...
    DecoderFixTs(p_dec, &p_audio->i_pts, NULL, &p_audio->i_length,
                 &i_rate, AOUT_MAX_ADVANCE_TIME, 0);
    vlc_mutex_unlock(&p_owner->lock);

    audio_output_t *p_aout = p_owner->p_aout;

    if (p_aout != NULL && p_audio->i_pts > VLC_TS_INVALID && i_rate >= INPUT_RATE_DEFAULT / AOUT_MAX_INPUT_RATE && i_rate <= INPUT_RATE_DEFAULT * AOUT_MAX_INPUT_RATE && !DecoderTimedWait(p_dec, p_audio->i_pts - AOUT_MAX_PREPARE_TIME))
    {
...
static void DecoderPlaySpu(decoder_t *p_dec, subpicture_t *p_subpic)
{
    ...
    if (p_subpic->i_start <= VLC_TS_INVALID || DecoderTimedWait(p_dec, p_subpic->i_start - SPU_MAX_PREPARE_TIME))
    {
        subpicture_Delete(p_subpic);
        return;
    }
...
Video functions 
****************************************************************/
static void DecoderMaskChangeRate_video(decoder_t *p_dec, int i_rate)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    input_clock_ChangeRate_video(p_owner->p_clock, i_rate);
}
static void DecoderMaskChangePause_video(decoder_t *p_dec, bool b_paused)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    mtime_t date = mdate();
    if (b_paused)
    {
        // input_thread_t *p_input = p_owner->p_input;
        // bool b_source_paused = input_priv(p_input)->b_can_pause;
...
static void DecoderMaskSync(decoder_t *p_dec)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
...
Audio functions 
****************************************************************/
static void DecoderMaskChangeRate_audio(decoder_t *p_dec, int i_rate)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    input_clock_ChangeRate_audio(p_owner->p_clock, i_rate);
}
static void DecoderMaskChangePause_audio(decoder_t *p_dec, bool b_paused)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    mtime_t date = mdate();
    if (b_paused)
    {
        ...
        // assert( !b_source_paused == !b_paused );
        input_DecoderChangePause(p_dec, true, date);
        input_clock_ChangePause_audio(p_owner->p_clock, true, date);
    }
    else
    {
        input_DecoderChangePause(p_dec, false, date);
        input_clock_ChangePause_audio(p_owner->p_clock, false, date);
    }
}
...
static inline void DecoderControlPush(decoder_t *p_dec, enum input_control_e es_op, vlc_value_t* val)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    input_thread_t *p_input = p_owner->p_input;
    input_ControlPush(p_input,es_op,val);
}

static double CalcRate(char* expression,double t, decoder_t *p_logger)
{
    msg_Dbg(p_logger, "Calculating expression %s at t=%lf",expression,t);
...
static void DecoderInsertNewSegVideo(decoder_t *p_dec, char p_start[MAX_LINE_LEN],char p_end[MAX_LINE_LEN],char p_audio_func[MAX_LINE_LEN],char p_video_func[MAX_LINE_LEN],char p_sync[MAX_LINE_LEN],char p_audio_pause[MAX_LINE_LEN], char p_video_pause[MAX_LINE_LEN])
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    msg_Dbg(p_dec,"Video: Read new segment");
    p_owner->i_seg_start = atoi(p_start);
    p_owner->i_seg_end = atoi(p_end);  
    strcpy(p_owner->psz_rate_func,p_video_func);
...
static void DecoderInsertNewSegAudio(decoder_t *p_dec, char p_start[MAX_LINE_LEN],char p_end[MAX_LINE_LEN],char p_audio_func[MAX_LINE_LEN],char p_video_func[MAX_LINE_LEN],char p_sync[MAX_LINE_LEN],char p_audio_pause[MAX_LINE_LEN], char p_video_pause[MAX_LINE_LEN])
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    msg_Dbg(p_dec,"Audio: Read new segment");
    p_owner->i_seg_start = atoi(p_start);
    p_owner->i_seg_end = atoi(p_end);  
    strcpy(p_owner->psz_rate_func,p_audio_func);
...
static void DecoderApplyMask(decoder_t *p_dec)
{
    ...
    if (p_dec->fmt_out.i_cat == VIDEO_ES)
    {
        ...
        else if (i_last_stream < i_seg_start)
        {
            ...
            DecoderInsertNewSegVideo(p_dec,p_start,p_end,p_audio_func,p_video_func,p_sync,p_audio_pause,p_video_pause);
            p_owner->i_last_stream = i_last_stream;
        }
    ...
    else //AUDIO_ES or SPU_ES
    {
        ...
        mtime_t i_seg_start = p_owner->i_seg_start;
        mtime_t i_seg_end = p_owner->i_seg_end;
        char p_start[MAX_LINE_LEN], p_end[MAX_LINE_LEN], p_audio_func[MAX_LINE_LEN],\
                     p_video_func[MAX_LINE_LEN], p_sync[MAX_LINE_LEN], p_audio_pause[MAX_LINE_LEN],\
                     p_video_pause[MAX_LINE_LEN];
        ...
        {
            msg_Dbg(p_dec,"Audio: Entering seek");
            fseek(p_owner->p_maskfile,0,SEEK_SET);
            while(fscanf(p_owner->p_maskfile,"%s %s %s %s %s %s %s\n",p_start,p_end,p_audio_func,p_video_func,p_sync,p_audio_pause,p_video_pause))
            {
                int i_start = atoi(p_start);
                int i_end = atoi(p_end);
                if (i_last_stream >= i_start && i_last_stream < i_end){
                    msg_Dbg(p_dec,"Audio: found from seek");
                    break;
                }
                
            }
            DecoderInsertNewSegAudio(p_dec,p_start,p_end,p_audio_func,p_video_func,p_sync,p_audio_pause,p_video_pause);
            p_owner->i_last_stream = i_last_stream;
        }
                                ...
                                sout_instance_t *p_sout)
{
    ...
    p_owner->b_paused_mask = false;
    p_owner->b_sync = false;
    p_owner->b_sync_after_pause = false;

    char *psz_path = var_InheritString(p_dec, "mask");
    if (strcmp(psz_path, "none") == 0)
        p_owner->p_maskfile = NULL;
    else
    {
        p_owner->p_maskfile = fopen(psz_path, "r");
        if (unlikely(p_owner->p_maskfile == NULL))
        {
            free(p_owner);
            vlc_object_release(p_dec);
            return NULL;
